...
int detect(char *model_path, char *image_path, char *save_image_path)
{
    ...
    // 将检测结果按照classId、score、pt1x、pt1y、pt2x、pt2y、pt3x、pt3y、pt4x、pt4y 的格式存放在vector<float>中
    GetResultRectYolov8obb PostProcess;
    ...
    for (int i = 0; i < DetectiontRects.size(); i += 10)
    {
        ...
        cv::line(src_image, cv::Point(pt1x, pt1y), cv::Point(pt2x, pt2y), cv::Scalar(0, 255, 0), 2);
        cv::line(src_image, cv::Point(pt2x, pt2y), cv::Point(pt3x, pt3y), cv::Scalar(0, 255, 0), 2);
        cv::line(src_image, cv::Point(pt3x, pt3y), cv::Point(pt4x, pt4y), cv::Scalar(0, 255, 0), 2);
        cv::line(src_image, cv::Point(pt4x, pt4y), cv::Point(pt1x, pt1y), cv::Scalar(0, 255, 0), 2);

        char text1[32];
        sprintf(text1, "%d:%.2f", classId, conf);
        putText(src_image, text1, cv::Point(pt1x, pt1y), cv::FONT_HERSHEY_SIMPLEX, 0.65, cv::Scalar(0, 0, 255), 2, cv::LINE_AA);
    }

    imwrite(save_image_path, src_image);

    printf("== obj: %d \n", int(float(DetectiontRects.size()) / 10.0));
...
{
    char model_path[256] = "/home/firefly/zhangqian/rknn/rknpu2_1.4.0/examples/rknn_yolov8_obb_demo/model/RK3588/yyolov8n-obb.rknn";
    char image_path[256] = "/home/firefly/zhangqian/rknn/rknpu2_1.4.0/examples/rknn_yolov8_obb_demo/test.jpg";
    char save_image_path[256] = "/home/firefly/zhangqian/rknn/rknpu2_1.4.0/examples/rknn_yolov8_obb_demo/test_result.jpg";
